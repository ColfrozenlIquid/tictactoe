...
Nought::Nought(SDL_Renderer* sdl_renderer) {
    m_renderer = sdl_renderer;
    m_sprite_index = get_Random_Int_Range(0, NOUGHT_SPRITE_COUNT-1);
    m_texture = load_Texture(m_sprite_index);
}

Nought::~Nought() {}

void Nought::blit_Texture() {
...
SDL_Texture* Nought::load_Texture(int index) {
    SDL_Texture* texture;
    std::string filepath = NOUGHT_SPRITE_PATH + std::to_string(index) + ".bmp";
...
int Nought::get_x_Pos() {
    return m_x_pos;
}

int Nought::get_y_Pos() {
    return m_y_pos;
}